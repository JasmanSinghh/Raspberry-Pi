...
private:
    struct Node
    {
        ...
        Node(int idx, const string& val, bool insertion)
            : index(idx), value(val), isInsertion(insertion), next(nullptr)
        {
        }
    };
    ...
    int sz;      // size
    Node* undoStack;          // pointer to the top of the undo stack
...
FIXME:
    // push to stack
    void pushToUndoStack(int index, const string& value, bool isInsertion)
    {
        Node* newNode = new Node(index, value, isInsertion);
...
public:
    Stringlist()
        : cap(10), arr(new string[cap]), sz(0), undoStack(nullptr)
    {
    }

    Stringlist(const Stringlist& other)
        : cap(other.cap), arr(new string[cap]), sz(other.sz), undoStack(nullptr)
    {
        copy(other.arr);
    }
    ...
    void insert_before(int index, const string& s)
    {
        ...
        pushToUndoStack(index, "", true); // Push the current list to the undo stack
    }
    ...
    void remove_at(int index)
    {
        ...
        pushToUndoStack(index, arr[index], false); // Push the current list to the undo stack
    }
    //
    ...
    bool undo()
    {
        ...
        Node* nodeToUndo = undoStack;
        undoStack = undoStack->next;

        if (nodeToUndo->isInsertion)
        {
            remove_at(nodeToUndo->index);
        }
        else
        {
            insert_before(nodeToUndo->index, nodeToUndo->value);
        }
